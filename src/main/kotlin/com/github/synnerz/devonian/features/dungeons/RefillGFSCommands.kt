...
object RefillGFSCommands {
    ...
    fun initialize() {
        DevonianCommand.command.subcommand("pearls") { _, args ->
            ...
            val amount = inv.sumOf { if ((it.customName?.string ?: it.displayName.string).contains("Ender Pearl")) it.count else 0 }
        ...
        DevonianCommand.command.subcommand("leaps") { _, args ->
            val inv = inventory ?: return@subcommand 0
            val amount = inv.sumOf { if (it.customName?.string?.contains("Spirit Leap") == true) it.count else 0 }
        ...
        DevonianCommand.command.subcommand("superbooms") { _, args ->
            val inv = inventory ?: return@subcommand 0
            val amount = inv.sumOf { if (it.customName?.string?.contains("Superboom TNT") == true) it.count else 0 }
