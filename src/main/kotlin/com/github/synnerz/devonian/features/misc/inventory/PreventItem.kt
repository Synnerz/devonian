...
object PreventItem {
    ...
    fun initialize() {
        ...
        EventBus.on<PickupItemInventoryEvent> { event ->
            ...
            val type = BadScreen.getType(event.screen)
            val evn = SlotEvent(slot, slot.item, slot.containerSlot, type != null, event)
            if (!evn.post()) return@on

            event.cancel()
            onCancel("&cPrevented ${type?.action ?: "moving"} item", evn.actors)
        }
        ...
        EventBus.on<QuickCraftMoveEvent> { event ->
            ...
            event.cancel()
            onCancel("&cPrevented ${type?.action ?: "placing"} item", evn.actors)
        }
        ...
        EventBus.on<SwapItemEvent> { event ->
            ...
            val type = BadScreen.getType(event.screen)
            val evn1 = SlotEvent(slot1, slot1.item, slot1.containerSlot, type != null, event, slot2, false)
            val evn2 = SlotEvent(slot2, slot2.item, slot2.containerSlot, type != null, event, slot1, true)
            if (!evn1.post() && !evn2.post()) return@on

            event.cancel()
            onCancel("&cPrevented ${type?.action ?: "moving"} item", (evn1.actors.toSet() + evn2.actors).toList())
        }
    }

    enum class BadScreen(val action: String) {
        Salvage("salvaging") {
        ...
        Trade("trading") {
            val titleReg = "^You\\s+\\w+$".toRegex()

            override fun matches(screen: AbstractContainerScreen<*>): Boolean {
                return titleReg.matches(screen.title.string)
            }
        },
        Sell("selling") {
            override fun matches(screen: AbstractContainerScreen<*>): Boolean {
        ...
        abstract fun matches(screen: AbstractContainerScreen<*>): Boolean

        companion object {
            fun getType(screen: AbstractContainerScreen<*>) = entries.find { it.matches(screen) }
        }
    }
