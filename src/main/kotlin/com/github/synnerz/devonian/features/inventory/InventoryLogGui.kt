...
object InventoryLogGui : Feature("inventoryLogGui") {
    ...
    data class ItemizedDifference(val name: String, var quantity: Int) {
        ...
        fun matches(n: String, q: Int) = (n == name && q.sign == quantity.sign)
        ...
        override fun toString(): String = "${if (quantity < 0) "&c" else "&a"}${"%,d".format(quantity)}&r $name"
    ...
    override fun initialize() {
        on<TickEvent> {
            ...
            val changeCount = inv.changeCount
            if (changeCount == lastChangeCount) return@on
            lastChangeCount = changeCount

            val newInv = mutableMapOf<String, Int>()
            inv.forEachIndexed { i, v ->
                if (i == PlayerInventory.OFF_HAND_SLOT) return@on
